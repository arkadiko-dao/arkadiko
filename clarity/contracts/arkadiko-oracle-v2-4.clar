...
(define-constant ERR-STALE-MESSAGE u8404)
...
(define-public (update-price-owner (token-id uint) (price uint) (decimals uint))
  (begin
    ...
    (unwrap-panic (update-price-multi-helper token-id price decimals burn-block-height))
    (ok price)
  )
)
...
(define-public (update-price-multi (block uint) (token-id uint) (price uint) (decimals uint) (signatures (list 10 (buff 65))))
  (let (
    ...
    (if (>= check-result (var-get minimum-valid-signers))
      (update-price-multi-helper token-id price decimals block)
      (ok false)
    )
  )
)
...
(define-private (update-price-multi-helper (token-id uint) (price uint) (decimals uint) (block uint))
  (let (
    ...
    (blocks-list (list block block block block))
  )
    (map update-price-token-iter names-list prices-list decimals-list blocks-list)
    (ok true)
  )
)

;; Helper method to iterate over all token names and update prices
(define-private (update-price-token-iter (token (string-ascii 12)) (price uint) (decimals uint) (block uint))
  (begin
    (map-set prices { token: token } { last-price: price, last-block: block, decimals: decimals })
    (ok true)
  )
)

;; Get the highest last-block across all token names for a token ID
(define-read-only (get-token-last-block (token-id uint))
  (fold get-token-last-block-iter (get-token-names-from-id token-id) u0)
)

(define-private (get-token-last-block-iter (token (string-ascii 12)) (current-max uint))
  (let ((token-block (get last-block (get-price token))))
    (if (> token-block current-max) token-block current-max)
  )
)
...
;; Verify signature and check if signer is trusted
;; Uses secp256k1-verify to reject malleable signatures
(define-read-only (check-price-signer (block uint) (token-id uint) (price uint) (decimals uint) (signature (buff 65)))
  (let (
    (message-hash (get-signable-message-hash block token-id price decimals))
    (pubKey (unwrap! (pubkey-price-signer block token-id price decimals signature) u0))
  )
    (if (and (is-trusted-oracle pubKey) (secp256k1-verify message-hash signature pubKey))
      u1
      u0
    )
  )
