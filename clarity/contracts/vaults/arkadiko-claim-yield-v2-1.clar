  ...
  (let (
    ...
    (if (is-eq stack-yield true)
      (begin
        ...
        ;;   (try! (contract-call? .arkadiko-stacker-v3-1 enable-vault-withdrawals vault-id))
        ;;   (if (is-eq stacker-name "stacker-2")
        ;;     (try! (contract-call? .arkadiko-stacker-2-v3-1 enable-vault-withdrawals vault-id))
        ;;     (if (is-eq stacker-name "stacker-3")
        ;;       (try! (contract-call? .arkadiko-stacker-3-v3-1 enable-vault-withdrawals vault-id))
        ;;       (if (is-eq stacker-name "stacker-4")
        ;;         (try! (contract-call? .arkadiko-stacker-4-v3-1 enable-vault-withdrawals vault-id))
        ;;         false
        ;;       )
        ;;     )
        ;;   )
