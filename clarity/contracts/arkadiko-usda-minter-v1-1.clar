...
(define-public (mint-usda (oracle <oracle-trait>) (amount uint))
  (let (
    ...
    (decimals u1000000)
    (amount-to-burn-per-usda (/ (/ (* u100 decimals) (get last-price diko-price)) u100)) ;; TODO: fix when DIKO price is > $1
...
(define-public (burn-usda (oracle <oracle-trait>) (amount uint))
  (let (
    (diko-price (unwrap-panic (contract-call? oracle fetch-price "diko")))
    (decimals u1000000)
    (amount-to-mint-per-usda (/ (/ (* u100 decimals) (get last-price diko-price)) u100)) ;; TODO: fix when DIKO price is > $1
    ...
    (var-set current-supply (- (var-get current-supply) amount)) ;; TODO: fix if negative number
    (ok true)
  )
)
