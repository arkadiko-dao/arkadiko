...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // Add rewards
    result = liquidationRewards.addReward(1, 0, "escrowed-diko-token", 100);
    result.expectOk().expectBool(true);

    // Add rewards
    result = liquidationRewards.addReward(2, 0, "escrowed-diko-token", 300);
    result.expectOk().expectBool(true);
    ...
    // Claim reward
    result = liquidationRewards.claimRewards(deployer, 0, "escrowed-diko-token");
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    result = liquidationRewards.addReward(1, 50, "escrowed-diko-token", 100);
    result.expectOk().expectBool(true);

    // Add rewards - unlock block 100
    result = liquidationRewards.addReward(2, 100, "escrowed-diko-token", 300);
    result.expectOk().expectBool(true);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let liquidationPool = new LiquidationPool(chain, deployer);
    let liquidationRewards = new LiquidationRewards(chain, deployer);
    let liquidationRewardsEsDiko = new LiquidationRewardsEsDiko(chain, deployer);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // Update epoch
    result = liquidationRewardsEsDiko.updateEpoch(0.2, 100, 100);
    result.expectOk().expectBool(true);

    // Reward data
    call = await liquidationRewardsEsDiko.getEpochInfo();
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let result = liquidationRewards.addReward(1, 0, "escrowed-diko-token", 100);
    result.expectOk().expectBool(true);

    // Disable DIKO
    result = liquidationRewards.updateRewardToken("escrowed-diko-token", false)
    result.expectOk().expectBool(true);
    ...
    result = liquidationRewards.addReward(1, 0, "escrowed-diko-token", 100, true);
    result.expectOk().expectBool(true);

    // Enable DIKO again
    result = liquidationRewards.updateRewardToken("escrowed-diko-token", true)
    result.expectOk().expectBool(true);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // Toggle shutdown
    result = liquidationRewardsEsDiko.toggleEmergencyShutdown();
    result.expectOk().expectBool(true);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    call.result.expectOk().expectTuple()["token"].expectPrincipal(Utils.qualifiedName("escrowed-diko-token"));
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    // Start balance = 10k diko
    call = esDikoToken.balanceOf(wallet_1.address);
    call.result.expectOk().expectUintWithDecimals(10000);  

    // Claim all at once
    result = liquidationUI.claimRewards(wallet_1.address, rewardIds, "escrowed-diko-token", false);
    result.expectOk().expectBool(true)

    // Balance = 10k initial + 100 * 19
    call = esDikoToken.balanceOf(wallet_1.address);
    call.result.expectOk().expectUintWithDecimals(11900);  
  }
});
