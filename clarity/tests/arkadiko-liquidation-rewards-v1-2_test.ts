...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    call.result.expectTuple()["share-block"].expectUint(5);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    result = liquidationRewards.addReward(5, 50, "arkadiko-token", 100);
    result.expectOk().expectBool(true);

    // Add rewards - unlock block 100
    result = liquidationRewards.addReward(6, 100, "arkadiko-token", 300);
    result.expectOk().expectBool(true);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    call.result.expectTuple()["share-block"].expectUint(13);
