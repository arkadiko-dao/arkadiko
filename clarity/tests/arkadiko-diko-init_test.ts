...
Clarinet.test({
  name: "diko-init: migrate to diko-init-v2-1",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    let deployer = accounts.get("deployer")!;
    let wallet_1 = accounts.get("wallet_1")!;

    // First contract was deployed on block 34246
    // Move to 1 month after that
    chain.mineEmptyBlockUntil(34246 + 144 * 30);

    let call = await chain.callReadOnlyFn("arkadiko-token", "get-balance", [
      types.principal(Utils.qualifiedName("arkadiko-diko-init")),
    ], deployer.address);
    ...
    // Try to claim - should fail
    block = chain.mineBlock([
      Tx.contractCall("arkadiko-diko-init-v2-1", "migrate", [
      ], deployer.address)
    ]);
    block.receipts[0].result.expectOk().expectBool(true);
    ...
    call = await chain.callReadOnlyFn("arkadiko-token", "get-balance", [
      types.principal(Utils.qualifiedName("arkadiko-diko-init-v2-1")),
    ], deployer.address);
    call.result.expectOk().expectUintWithDecimals(50000000 - 50000)

    // Can claim
    block = chain.mineBlock([
      Tx.contractCall("arkadiko-diko-init-v2-1", "founders-claim-tokens", [
          types.uint(437500 * 1000000)
      ], deployer.address)
    ]);
    block.receipts[0].result.expectOk().expectBool(true);
  }
