...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    block = chain.mineBlock([
      Tx.contractCall("arkadiko-stacker-payer-v2-1", "redeem-stx-helper", [
        types.uint(1000000000), // 1000 STX
      ], wallet_2.address)
    ]);
    let [_, burn_event, transfer_event] = block.receipts[0].events;
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    call = await chain.callReadOnlyFn("arkadiko-stacker-payer-v2-1", "get-stx-redeemable", [], deployer.address);
    call.result.expectUintWithDecimals(100);

    // Total of 850 STX redeemable
    call = await chain.callReadOnlyFn("arkadiko-stacker-payer-v2-1", "get-stx-redeemable-helper", [], deployer.address);
