...
async fn(chain: Chain, accounts: Map<string, Account>) {
  ...
  let call = chain.callReadOnlyFn("arkadiko-diko-guardian-v2-1", "get-staking-rewards-per-burn-block", [], wallet_1.address);
  call.result.expectUintWithDecimals(626.399062)

  // Get rewards after 13 steps
  chain.mineEmptyBlock(2016 * 13);
  call = chain.callReadOnlyFn("arkadiko-diko-guardian-v2-1", "get-staking-rewards-per-burn-block", [], wallet_1.address);
