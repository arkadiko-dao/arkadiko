...
Clarinet.test({
  name: "usda: burn mf burn",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    let deployer = accounts.get("deployer")!;    

    let block = chain.mineBlock([
      Tx.contractCall("arkadiko-governance-v1-1", "add-contract-address", [
        types.ascii("arkadiko-burn-usda"),
        types.principal(deployer.address),
        types.principal(Utils.qualifiedName("arkadiko-burn-usda")),
        types.bool(true),
        types.bool(true)
      ], deployer.address)
    ]);
    ...
    let call = await chain.callReadOnlyFn("usda-token", "get-balance", [
      types.principal('ST1QV6WVNED49CR34E58CRGA0V58X281FAS1TFBWF'),
    ], deployer.address);
    call.result.expectOk().expectUint(1000000000000);

    block = chain.mineBlock([
      Tx.contractCall("arkadiko-burn-usda", "burn-mf", [], deployer.address)
    ]);
    block.receipts[0].result.expectOk().expectBool(true);
    ...
    call = await chain.callReadOnlyFn("usda-token", "get-balance", [
      types.principal('ST2JHG361ZXG51QTKY2NQCVBPPRRE2KZB1HR05NNC'),
    ], deployer.address);
    call.result.expectOk().expectUint(1000000000010);
  }
});

// TODO
// Clarinet.test({
//   name: "swap: burn malicious tokens",
//   async fn(chain: Chain, accounts: Map<string, Account>) {
//     let deployer = accounts.get("deployer")!;
//   }
// });
