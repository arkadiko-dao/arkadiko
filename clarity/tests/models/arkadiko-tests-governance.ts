...
class Governance {
  ...
  getAllProposals() {
    return this.chain.callReadOnlyFn("arkadiko-governance-v3-1", "get-proposals", [], this.deployer.address);
  }

  getAllProposalIDs() {
    return this.chain.callReadOnlyFn("arkadiko-governance-v3-1", "get-proposal-ids", [], this.deployer.address);
  }

  getProposalByID(id: number) {
    return this.chain.callReadOnlyFn("arkadiko-governance-v3-1", "get-proposal-by-id", [types.uint(id)], this.deployer.address)
  }

  getMemberVotes(id: number, member: Account) {
    return this.chain.callReadOnlyFn("arkadiko-governance-v3-1", "get-votes-by-member-by-id", [
      types.uint(id), 
  ...
  getMemberVoteTokens(id: number, member: Account, token: string = "arkadiko-token") {
    return this.chain.callReadOnlyFn("arkadiko-governance-v3-1", "get-tokens-by-member-by-id", [
      types.uint(id), 
      types.principal(member.address),
  ...
  createProposalDao(user: Account, startBlock: number, title: string, url: string, contractChanges: string[]) {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-governance-v3-1", "propose-dao", [
        types.principal(Utils.qualifiedName('arkadiko-stake-pool-diko-v1-2')),
        types.uint(startBlock),
        types.utf8(title),
        types.utf8(url),        
  ...
  voteForProposal(user: Account, proposal: number, amount: number, token: string = "arkadiko-token") {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-governance-v3-1", "vote-for", [
        types.principal(Utils.qualifiedName('arkadiko-stake-pool-diko-v1-2')),
        types.principal(Utils.qualifiedName(token)),
        types.uint(proposal),
  ...
  voteAgainstProposal(user: Account, proposal: number, amount: number, token: string = "arkadiko-token") {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-governance-v3-1", "vote-against", [
        types.principal(Utils.qualifiedName('arkadiko-stake-pool-diko-v1-2')),
        types.principal(Utils.qualifiedName(token)),
        types.uint(proposal),
        types.uint(amount * 1000000)
  ...
  endProposal(proposal: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-governance-v3-1", "end-proposal", [
        types.uint(proposal)
      ], this.deployer.address)
    ]);
    return block.receipts[0].result;
  ...
  toggleShutdown() {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-governance-v3-1", "toggle-governance-shutdown", [], this.deployer.address)
    ]);
    return block.receipts[0].result;
  }
