...
class Governance {
  ...
  getAllProposals() {
    return this.chain.callReadOnlyFn("arkadiko-governance-v4-2", "get-proposals", [], this.deployer.address);
  }

  getAllProposalIDs() {
    return this.chain.callReadOnlyFn("arkadiko-governance-v4-2", "get-proposal-ids", [], this.deployer.address);
  }

  getProposalByID(id: number) {
    return this.chain.callReadOnlyFn("arkadiko-governance-v4-2", "get-proposal-by-id", [types.uint(id)], this.deployer.address)
  }

  getMemberVotes(id: number, member: Account) {
    return this.chain.callReadOnlyFn("arkadiko-governance-v4-2", "get-votes-by-member-by-id", [
      types.uint(id), 
  ...
  getMemberVoteTokens(id: number, member: Account, token: string = "arkadiko-token") {
    return this.chain.callReadOnlyFn("arkadiko-governance-v4-2", "get-tokens-by-member-by-id", [
      types.uint(id), 
      types.principal(member.address),
  ...
  endProposal(proposal: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-governance-v4-2", "end-proposal", [
        types.uint(proposal)
      ], this.deployer.address)
    ]);
    return block.receipts[0].result;
  ...
  toggleShutdown() {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-governance-v4-2", "toggle-governance-shutdown", [], this.deployer.address)
    ]);
    return block.receipts[0].result;
  }
