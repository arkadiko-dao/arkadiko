...
class VaultManager {
  ...
  toggleStacking(user: Account, vaultId: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-pox-unstack-unlock-v2-5", "unstack", [
        types.uint(vaultId)
      ], user.address),
    ]);
    return block.receipts[0].result;
  ...
  redeemLotCollateralXstx(user: Account, auctionId: number = 1, lot: number = 0) {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-auction-engine-v2-1", "redeem-lot-collateral", [
        ...
        types.principal(
          Utils.qualifiedName('arkadiko-collateral-types-v3-1'), 
        ),
        types.uint(auctionId),
        types.uint(lot)
      ], user.address)
...
class VaultAuctionV4 {
  ...
  startAuction(user: Account, vaultId: number, token: string = 'xstx-token', reserve: string = 'arkadiko-sip10-reserve-v2-1') {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-auction-engine-v4-5", "start-auction", [
        types.uint(vaultId),
        types.principal(Utils.qualifiedName('arkadiko-freddie-v1-1')),
        types.principal(Utils.qualifiedName('arkadiko-collateral-types-v3-1')),
        types.principal(Utils.qualifiedName('arkadiko-oracle-v3-0')),
  ...
  toggleEmergencyShutdown() {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-auction-engine-v4-5", "toggle-auction-engine-shutdown", [
      ], this.deployer.address)
    ]);
    return block.receipts[0].result;
  }
  ...
  updateFee(fee: number) {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-auction-engine-v4-5", "update-fee", [
        types.uint(fee),
      ], this.deployer.address)
    ]);
    return block.receipts[0].result;
  ...
  withdrawFees(token: string) {
    let block = this.chain.mineBlock([
      Tx.contractCall("arkadiko-auction-engine-v4-5", "withdraw-fees", [
        types.principal(Utils.qualifiedName(token)),
      ], this.deployer.address)
    ]);
    return block.receipts[0].result;
