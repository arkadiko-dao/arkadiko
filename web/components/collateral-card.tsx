  ...
  return (
    <>
      {collateralItems.map((collateral) => (
        <div key={collateral.tokenType} className={`group border border-${collateral.token}/5 hover:border-${collateral.token}/10 shadow-md shadow-${collateral.token}/10 flex flex-col col-span-1 bg-gradient-to-br from-${collateral.token}/5 to-${collateral.token}/10 divide-y divide-gray-200 rounded-md transition duration-700 ease-in-out`}>
