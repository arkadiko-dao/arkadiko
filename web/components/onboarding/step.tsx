    ...
    >
      <div className={`absolute ${stepPosition}`}>
        <span className="flex w-6 h-6">
          ...
          <span className="relative inline-flex items-center justify-center w-6 h-6 text-xs font-semibold text-white bg-gray-800 rounded-full">
            {stepNumber}
          </span>
        </span>
      </div>
      ...
      >
        <div className="relative p-6 mx-auto">
          <div className="flex items-center">
            ...
            <h2 className="text-lg text-white font-headings">{stepTitle}</h2>
          </div>
          <p className="mt-3 text-sm text-gray-300">{stepDescription}</p>
      ...
      <svg
        aria-hidden="true"
        className={`absolute transform ${arrowSize} ${arrowPosition}`}
      ...
      >
        <path
          ...
          fill="none"
          className={classNames(
            currentStep >= stepNumber ? 'draw-arrow' : '',
            'text-gray-800 stroke-current'
          )}
        ...
        <path
          ...
          fill="none"
          className={classNames(
            currentStep >= stepNumber ? 'draw-arrow tail-1' : '',
            'text-gray-800 stroke-current'
          )}
        ...
        <path
          ...
          fill="none"
          className={classNames(
            currentStep >= stepNumber ? 'draw-arrow tail-2' : '',
            'text-gray-800 stroke-current'
          )}
